...
/**
 * Server Component - Displays the approximate capital required to purchase an American Gold Eagle.
 * 
 * Uses American Eagle (AE) pricing from Monex API.
...
export default async function CapitalRequirementsCard({ priceData }: CapitalRequirementsCardProps = {}) {
  // Use provided data or fetch AE (American Eagle) if not available
  ...
  return (
    <div className="card p-8 max-w-sm">
      ...
      <p className="text-gray-400 text-sm text-center mb-6">
        {hasValidPrice
          ? `Based on current Gold Eagle (${SITE_CONFIG.productSymbol}) ask price:`
          : "To purchase a single American Gold Eagle:"}
      </p>
      <div className="text-center py-6 rounded-lg bg-bullion-gold/10 border border-bullion-gold/30">
        {hasValidPrice ? (
          <>
            ...
            <p className="text-gray-400 text-sm mt-2">per 1 oz Gold Eagle</p>
          </>
        ) : (
          <>
